...
/* The survivor test lives in ../verify.js, which the browser imports too.
   All this does is hand it the two things the halves cannot share: a
   Wikidata client that sends a User-Agent and retries on 429, and a TMDB
   client that knows where the key is kept. The reasoning is not ours.
  ...
  return survivors({
    ...
    tmdb: async path => {
      try {
        ...
        /* Null, not a throw — the caller counts an unanswerable person as
           unknown, which is the honest reading of a failed lookup. */
        return null;
      }
    },
  });
